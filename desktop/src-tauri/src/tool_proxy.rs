        ...
        match listener.accept().await {
            Ok((stream, addr)) => {
                ...
                tokio::spawn(async move {
                    handle_proxy_connection(stream, bridge, app, plan_approval).await;
                });
            }
            Err(e) => {
...
async fn handle_proxy_connection(
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        ...
        let response = if tool == "propose_plan" {
            handle_propose_plan(&app, &plan_approval, &args).await
        } else {
            log::info!("Tool proxy: dispatching '{}' to bridge", tool);
            match bridge.send_tool_request(tool, args).await {
                Ok(payload) => payload,
